...
class Game {
  ...
  goToNextLevel() {
    this.levelIndex++;
    if (!this.levelWon()) {
      console.log(`ranking: ${true}`);
      this.loss();
    } else if (this.levelIndex < this.levels.length) {
      console.log(`ranking: ${false}`);
      this.startLevel();
    } else {
      console.log(`ranking: ${true}`);
      this.win();
  ...
  getScoreMessage() {
    ...
    localStorage.setItem("currentPlayerScore", this.score);
    console.log(`porcentagem: ${percentage}`);

    if (percentage === 100) {
      scoreMessage = 'Uma vitória de honra!.';
    }

    if (percentage < 100) {
      scoreMessage = 'Uma vitória com perdas!.';
    }

    if (percentage <= 95) {
      scoreMessage = 'Uma vitória quase perfeita.';
    }

    if (percentage <= 85) {
      scoreMessage = 'Uma guerra bem batalhada.';
    }

    if (percentage <= 75) {
      scoreMessage = 'Perdeu, mas com honra!.';
    }

    if (percentage <= 63) {
      scoreMessage = 'Perdeu e perdeu feio ainda!';
    }
